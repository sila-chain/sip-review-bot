...
import { Config, File, PullRequestData, RuleProcessed } from "./types.js";
...
async function run() {
    ...
    let pull_request: PullRequestData = payload.pull_request as PullRequest;
    ...
    core.info(
        `Running sip-review-bot on ${repository.owner.login}/${repository.name}#${pull_number} by "@${pull_request?.user?.login}"`,
    );
    ...
    if (previous_comment) {
        previous_comment = (
            await octokit.rest.issues.updateComment({
                ...
                body: "SIP Review Bot is running...",
            })
        ).data;
    } else {
        previous_comment = (
    ...
    try {
        ...
        if (
            ...
            const body =
                '❌ PR does not have "Allow edits from maintainers" enabled. This is required for the SIP Review Bot to function. Please enable it.';
            previous_comment = (
                await octokit.rest.issues.updateComment({
                    owner: repository.owner.login,
                    repo: repository.name,
        ...
        const cloneDir = await fs.mkdtemp(
            path.join(os.tmpdir(), "sip-review-bot-"),
        );
        ...
        // Pull and parse config file ('sip-editors.yml') from main branch of main repository using only isomorphic-git (no fs)
        let config: Config | undefined;
        try {
            const configFilePath = core.getInput("config") || "sip-editors.yml";
            ...
            config = parse(configText) as Config;
            ...
            try {
                ...
                previous_comment = (
                    await octokit.rest.issues.updateComment({
                        ...
                        repo: repository.name,
                        comment_id: previous_comment.id,
                        body: `🛑 Auto merge failed. Please see logs for more details, and report this issue at the [\`sip-review-bot\` repository](https://github.com/sila-chain/sip-review-bot).`,
                    })
                ).data;
        ...
        await octokit.rest.issues.updateComment({
            ...
            repo: repository.name,
            comment_id: previous_comment.id,
            body: `🛑 \`sip-review-bot\` failed for an unknown reason. Please see logs for more details, and report this issue at the [\`sip-review-bot\` repository](https://github.com/sila-chain/sip-review-bot).`,
        });
        throw e;
    }
}
